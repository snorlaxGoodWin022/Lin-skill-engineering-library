          ...
          h(
            ...
            },
            {
              reference: () =>
                h(
                  ElButton,
                  { type: 'danger', link: true, size: 'small' },
                  () => '删除'
                ),
            }
          ),
...
const router = useRouter()
const { list, total, params, isLoading, updateParams, resetParams, refetch } =
  useListData()
...
export function useListQuery<T, P extends Record<string, any>>(
...
export function useMutationAction<T, P>(options: UseMutationOptions<T, P>) {
  const queryClient = useQueryClient()
  const { mutationFn, queryKey, successMessage, errorMessage, onSuccess } =
    options
...
const handleBatchDelete = async () => {
  try {
    await ElMessageBox.confirm(
      `确认删除选中的 ${selectedRows.value.length} 条数据？`,
      '批量删除',
      { type: 'warning' }
    )
