...
/** 语言包命名空间 */
export type Namespace = 'common' | 'validation' | 'menu' | 'user' | 'order'
```
...
export function createValidationSchema() {
  ...
  return z.object({
    ...
    phone: z.string().regex(/^1[3-9]\d{9}$/, t('phone')),
  })
}
```
...
export function createValidationSchema() {
  ...
  return z.object({
    ...
    phone: z.string().regex(/^1[3-9]\d{9}$/, t('validation:phone')),
  })
}
```
...
<template>
  <el-select :model-value="locale" @change="changeLocale" style="width: 120px">
    <el-option
