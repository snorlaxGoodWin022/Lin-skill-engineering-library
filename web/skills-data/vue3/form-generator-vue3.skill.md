...
export const formSchema = z.object({
  // 必填字段
  username: z.string().min(1, '请输入用户名').max(20, '用户名不能超过20个字符'),
  ...
  // 数字范围
  age: z.number().min(18, '年龄不能小于18岁').max(100, '请输入正确的年龄'),
...
<template>
  <el-form label-position="top">
    <el-form-item label="用户名" :error="errors.username">
      <el-input v-model="usernameField.value.value" placeholder="请输入用户名" />
    </el-form-item>
  </el-form>
</template>
